...
SAVE_FILE = 'my_library.json' # Файл для локального сохранения библиотеки
...
ALL_FILES = [
    {
        ...
        'description': 'Бесплатный слот для вашего приложения',
        'screenshots': [],
        'link': ''
    },
    {
...
def load_library():
    ...
    try:
        ...
        # Восстанавливаем библиотеку на основе сохраненных имен файлов
...
def save_library():
    try:
        # Сохраняем только имена файлов, чтобы не дублировать массивы данных
...
class AppProfileView(AnimatedOverlay):
    ...
    def handle_download(self, sender):
        try:
            choice = console.alert('Download', f'Нажмите на Copy Link что бы скопировать ссылку на скачивание {self.file_data["name"]} и вставьте эту ссылку в любой браузер', 'Copy Link')
